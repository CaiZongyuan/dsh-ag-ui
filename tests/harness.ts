...
export async function mountGateway(
  ...
  const workspaceRoot = options.workspaceRoot ?? (options.persistenceRoot === undefined ? await mkdtemp(join(tmpdir(), 'ag-ui-workspaces-')) : join(options.persistenceRoot, 'workspaces'))
